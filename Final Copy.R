...
insurance %>%
  ...
  summarize(mean_charges = mean(charges)) %>%
  ungroup() %>%
  mutate(bmi_bins = reorder(bmi_bins, mean_charges)) -> bmi_order

ggplot(data = insurance, aes(x = bmi_bins, y = charges, fill = smoker)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_x_discrete(limits = bmi_order$bmi_bins) +
  facet_wrap(~smoker, ncol = 2) +
  labs(title = "Charges by BMI and Smoker Status") +
  theme_bw()
...
insurance %>%
  ...
  summarize(mean_charges = mean(charges)) %>%
  ungroup() %>%
  mutate(bmi_bins = factor(bmi_bins, levels = unique(bmi_bins[order(bmi)]))) -> bmi_order
...
clusplot(X_df, kmeans_model$cluster, color=TRUE, shade=TRUE, labels=0, lines=0)
...
#silhuotte
kResult <- pam(X_df,2,metric="euclidean")
kResult
...
kResult$clustering
summary(kResult$clustering)
plot(kResult)

spam <- silhouette(kResult, scale = 100)
plot(spam)
...
# Plot the silhouette
plot(silhouette(kResult$clustering), col = kResult$clustering)


# Perform K-means clustering on your data
kResult <- kmeans(X_df, centers = 5)

# Calculate the silhouette width for each data point
sil_width <- silhouette(kResult$cluster, dist(X))
...
pred_rf2 <- predict(rf.fit, newdata = test_pca)
plot(rf_fit2)


##comparison
pred_rf <- predict(rf.fit, newdata = test_pca)
plot(rf_fit2)
...
train_data <- data.frame(Y = rnorm(100), X1 = rnorm(100), X2 = rnorm(100), X3 = rnorm(100))

test_data <- data.frame(Y = rnorm(50), X1 = rnorm(50), X2 = rnorm(50), X3 = rnorm(50))



# Train random forest model without PCA
...
pca_model <- prcomp(train_data[, -1], scale. = TRUE)

X_pca_train <- predict(pca_model, train_data[, -1])

train_data_pca <- data.frame(Y = train_data$Y, X_pca_train)

rf_pca_model <- randomForest(Y ~ ., data = train_data_pca)

rf_pca_model

# Predict on test data and calculate ROC AUC for both models

pred_rf <- predict(rf_model, test_data[-1])

auc_rf <- multiclass.roc(test_data$Y, pred_rf)$auc



X_pca_test <- predict(pca_model, test_data[-1])

pred_rf_pca <- predict(rf_pca_model, as.data.frame(X_pca_test))

auc_rf_pca <- multiclass.roc(test_data$Y, pred_rf_pca)$auc



# Compare ROC AUC

cat("ROC AUC without PCA:", auc_rf, "\n")

cat("ROC AUC with PCA:", auc_rf_pca, "\n")



# Predict on test data and calculate MSE for both models
